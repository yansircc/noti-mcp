...
export async function sendWechatNotification({ title, description, apiKey }: WechatPushArgs) {
  try {
    ...
    if (!response.success) {
      ...
      return formatToolResponse("Invalid response format from ServerChan API", true);
    }
    
    const { data } = response;
    
    if (data.code === 0) {
      console.log(`ServerChan notification successful for key starting with ${apiKey.substring(0, 4)}...`);
      return formatToolResponse(`WeChat notification sent successfully. Message: ${data.message}`);
    }
    
    console.error(`ServerChan notification failed for key starting with ${apiKey.substring(0, 4)}... Code: ${data.code}, Message: ${data.message}`);
    return formatToolResponse(`Failed to send WeChat notification. Error: ${data.message} (Code: ${data.code})`, true);
  } catch (error) {
    console.error("Error sending wechat notification:", error);
    return formatToolResponse(`Error sending wechat notification: ${error instanceof Error ? error.message : String(error)}`, true);
  }
}
